...
local function get_ai_title_and_description(currentTitle)
  ...
  local success, diff_output, exit_code =
    execute_command("git diff --cached --no-prefix --diff-filter=AMDRC", true, true) -- return output, silent execution
...
%s
  ...
  local json_payload = string.format('{"contents": [{"parts":[{"text": %s}]}]}', escaped_prompt_for_json)
  ...
  if decoded.candidates and type(decoded.candidates) == "table" and #decoded.candidates > 0 then
    ...
    if
      ...
      and type(candidate.content.parts) == "table"
    ...
    vim.fn.prompt_setcallback(buf, function(input)
      ...
      if is_ai_request then
        ...
        if ai_title and ai_title ~= "AI Suggestion Failed" then
          ...
          vim.notify(
            "Failed to generate Gemini commit message or no staged changes found. Aborting.",
      ...
      local success_remote_check, remote_check_lines, _ =
        execute_command("git ls-remote --heads origin " .. escaped_branch_for_check, true, true)
      ...
      if success_push then
        ...
        vim.notify(
          "Push failed (exit code: " .. push_exit_code .. "). Please check git output or run 'git push' manually.",
          vim.log.levels.ERROR
        )
...
vim.api.nvim_create_user_command("GitCheckoutNewBranch", function()
...
vim.keymap.set("n", "<leader>gp", function()
  ...
  if success then
    ...
    vim.notify(
      "Pull failed (exit code: " .. exit_code .. "). Check git output or resolve conflicts.",
      vim.log.levels.ERROR
    )
    vim.notify("Output:\n" .. table.concat(output_lines, "\n"), vim.log.levels.ERROR)
  end
...
vim.api.nvim_create_user_command("GitPushCurrentBranch", function()
  -- Get current branch
  local success_branch, branch_output, _ = execute_command("git branch --show-current", true, true)
  if not success_branch or #branch_output == 0 then
  ...
  vim.notify("Checking remote status for branch: " .. current_branch, vim.log.levels.INFO)
  local success_remote_check, remote_check_lines, _ =
    execute_command("git ls-remote --heads origin " .. escaped_branch, true, true)
  local branch_exists_on_remote = success_remote_check and #remote_check_lines > 0
